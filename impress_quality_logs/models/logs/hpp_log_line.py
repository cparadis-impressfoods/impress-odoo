...
class HPPLogLine(models.Model):
    ...
    hpp_log_id = fields.Many2one(comodel_name='hpp.log', compute="_compute_hpp_log_id", store=True)
    ...
    _sql_constraints = [
        ('unique_cycle_number', 'UNIQUE(cycle_number, production_id)', 'Cycle Number must be unique for a production!'),
    ]

    cycle_number = fields.Integer('Cycle Number')
    cycle_time = fields.Selection(string='Cycle Time', selection=[('120sec','120 seconds'), ('300sec','300 seconds')])
    ...
    @api.depends('barrel_1_qty', 'barrel_2_qty', 'barrel_3_qty', 
                    'barrel_4_qty', 'is_cleaning_cycle')
    def _set_total_qty(self):
        self.ensure_one()
        self._cleaning_cycle_check()
        self.total_qty = self.barrel_1_qty + self.barrel_2_qty + self.barrel_3_qty + self.barrel_4_qty
    ...
    def _cleaning_cycle_check(self):
        for record in self:
            if record.is_cleaning_cycle:
                if record.barrel_1_qty != 0 or record.barrel_2_qty != 0 or record.barrel_3_qty != 0 or record.barrel_4_qty != 0:
                    raise ValidationError(_('Barrels must be empty during cleaning cycle'))
