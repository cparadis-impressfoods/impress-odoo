...
from odoo.addons.warehouse_billing.models.warehouse_billing_config import WarehouseBillingConfig
...
class GenerateWarehouseSaleOrder(models.TransientModel):
    ...
    def group_by_partner(self, configs) -> dict:
        ...
        for config in grouped_configs:
            grouped_configs[config] = self.env['warehouse.billing.config'].browse(grouped_configs[config])
    ...
    def get_configs_to_bill(self, check_date: date):
        ...
        config_list = config.search([('active', '=', True),('next_planned_invoice_date', '<=', check_date)])
        return config_list
    ...
    def action_generate_sale_order(self, configs):
        ...
        if not all(configs.mapped('billing_product_id')):
            raise UserError(_("Some billing configurations do not have a billing product defined."))
        ...
        quant_history = {config: group for config, group in quant_history.items() if len(group) != 0 or config.base_qty}

        # Check for groups with no sale_order_id
        quant_history = {config: group for config, group in quant_history.items() if not group.sale_order_id}
        ...
        for config,group in quant_history.items():
            ...
            sale_order_line_vals = self.generate_sale_order_line_values(config, group, today)
    ...
    @api.model
    def group_configs(self, configs: WarehouseBillingConfig) -> dict[list | int, WarehouseBillingConfig]:
        i = 0 
        grouped_configs = {}
        for config in configs:
    ...
    def generate_sale_orders(self, date=datetime.today().date()):
        ...
        while len(configs) != 0:
            ...
            for key in grouped_configs:
                sale_orders += config_model.with_context(warehouse_billing_date=date).bill_config(grouped_configs[key])
