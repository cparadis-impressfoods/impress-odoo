...
class PurchaseOrder(models.Model):
    ...
    billback_invoice_ids = fields.Many2many(
        "account.move", compute="_compute_billback_invoice_ids"
    )
    billback_invoice_count = fields.Integer(compute="_compute_billback_invoice_ids")

    def _compute_billback_invoice_ids(self):
        self.billback_invoice_ids = self.invoice_ids.filtered(
            lambda inv: inv.billback_invoice_id
        ).mapped(lambda inv: inv.billback_invoice_id.id)
    ...
    def action_view_billback_invoice(self, invoices=False):
        ...
        if len(invoices) > 1:  # type: ignore
            result["domain"] = [("id", "in", invoices.ids)]  # type: ignore
        elif len(invoices) == 1:  # type: ignore
            ...
            if "views" in result:
                ...
                result["views"] = form_view
            result["res_id"] = invoices.id  # type: ignore
        else:
