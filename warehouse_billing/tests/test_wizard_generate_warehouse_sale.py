...
class TestGenerateWarehouseSaleOrder(TransactionCase):
    ...
    def test_group_by_partner_1_partner(self):
        ...
        self.assertEqual(
            len(grouped_configs), 1, "All configs should be grouped into one partner"
        )
    ...
    def test_group_by_partner_2_partners(self):
        ...
        grouped_configs = self.wizard.group_by_partner(configs)
        self.assertEqual(
            len(grouped_configs), 2, "All configs should be grouped into 2 partners"
        )
    ...
    def test_get_configs_to_bill(self):
        ...
        configs_to_bill = config_model.create(
            [
                {
                    "name": "Config Monthly Last Day of Month",
                    "partner_id": self.partner_1.id,
                    "warehouse_id": self.warehouse.id,
                    "rate": 10.0,
                    "billing_product_id": self.billing_product.id,
                    "billing_cycle": "monthly",
                    "bill_last_day_month": True,
                    "last_invoice_date": datetime.strptime("2025-02-28", "%Y-%m-%d"),
                },
                {
                    "name": "Config Monthly Specific Day",
                    "partner_id": self.partner_1.id,
                    "warehouse_id": self.warehouse.id,
                    "rate": 10.0,
                    "billing_product_id": self.billing_product.id,
                    "billing_cycle": "monthly",
                    "billing_day_month": 22,
                    "last_invoice_date": datetime.strptime("2025-02-22", "%Y-%m-%d"),
                },
                {
                    "name": "Config Weekly",
                    "partner_id": self.partner_1.id,
                    ...
                    "billing_day_week": 1,
                    "last_invoice_date": datetime.strptime("2025-03-17", "%Y-%m-%d"),
                },
                {
                    "name": "Config Weekly late",
                    "partner_id": self.partner_1.id,
                    "warehouse_id": self.warehouse.id,
                    "rate": 10.0,
                    "billing_product_id": self.billing_product.id,
                    "billing_cycle": "weekly",
                    "billing_day_week": 1,
                    "last_invoice_date": datetime.strptime("2025-03-10", "%Y-%m-%d"),
                },
                {
                    "name": "Config Daily",
                    "partner_id": self.partner_1.id,
                    ...
                    "billing_product_id": self.billing_product.id,
                    "billing_cycle": "daily",
                    "last_invoice_date": datetime.strptime("2025-03-12", "%Y-%m-%d"),
                },
            ]
        )

        config_model.create(  # ignore
            [
                {
                    "name": "Config Monthly early",
                    "partner_id": self.partner_1.id,
                    "warehouse_id": self.warehouse.id,
                    "rate": 10.0,
                    "billing_product_id": self.billing_product.id,
                    "billing_cycle": "monthly",
                    "billing_day_month": 12,
                    "last_invoice_date": datetime.strptime("2025-03-12", "%Y-%m-%d"),
                },
                {
                    "name": "Config Weekly early",
                    "partner_id": self.partner_1.id,
                    "warehouse_id": self.warehouse.id,
                    "rate": 10.0,
                    "billing_product_id": self.billing_product.id,
