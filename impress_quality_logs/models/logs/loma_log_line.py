...
class LOMALogLine(models.Model):
    ...
    loma_log_id = fields.Many2one(comodel_name='loma.log', compute="_compute_loma_log_id", store=True)

    lower_limit = fields.Float('Lower Limit', related='loma_log_id.lower_limit', store=True, depends=["loma_log_id.lower_limit"])
    upper_limit = fields.Float('Upper Limit', related='loma_log_id.upper_limit', store=True, depends=["loma_log_id.upper_limit"])
    nominal_weight = fields.Float('Nominal Weight', related='loma_log_id.nominal_weight', store=True, depends=["loma_log_id.nominal_weight"])
