...
class QuantHistoryGroup(models.Model):
    ...
    config_id = fields.Many2one('warehouse.billing.config', string='Billing Configuration')
    quant_history_ids = fields.One2many('warehouse.quant.history', 'group_id', string='Quant History')

    invoicing_state = fields.Selection([
        ('not_invoiced', 'Not Invoiced'),
        ('partially_invoiced', 'Partially Invoiced'),
        ('invoiced', 'Invoiced'),
    ], string='Invoicing State', compute='_compute_invoicing_state', store=True)

    sale_order_id = fields.Many2one('sale.order', string='Sale Order', related="sale_order_line_id.order_id")
    sale_order_line_id = fields.Many2one('sale.order.line', string='Sale Order Line')

    invoiced = fields.Boolean(string='Invoiced', default=False, compute="_compute_invoiced", store=True) 
    invoice_ids = fields.Many2many('account.move', string='Invoice Reference', related="sale_order_id.invoice_ids")
    ...
    def _compute_invoiced(self):
        for record in self:
            if record.sale_order_line_id and record.sale_order_line_id.invoice_status == 'invoiced':
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name'):
                vals['name'] = self.env['ir.sequence'].next_by_code('warehouse.usage.group') or '/'
    ...
    def group_quant_history(self, config, today):
        ...
        if existing_group and len(existing_group) == 1:
            existing_group.write({
                'quant_history_ids': [(fields.Command.link(record.id)) for record in records],
            })
            return existing_group
        elif existing_group and len(existing_group) > 1:
            raise ValidationError(_("Multiple quant groups found for the same dates and configuration. Please delete the duplicates and try again."))
