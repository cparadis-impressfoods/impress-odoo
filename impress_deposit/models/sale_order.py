...
class SaleOrder(models.Model):
    ...
    deposit_value = fields.Monetary(string="Deposit", compute='_compute_deposit_value', store=True)
    ...
    def _handle_deposit(self):
        ...
        deposit_product_id = int(self.env['ir.config_parameter'].sudo().get_param('impress_deposit.deposit_product'))
        deposit_product = self.env['product.product'].browse(deposit_product_id)
        deposit_line = self.env['sale.order.line'].search([('order_id', '=',  self.id), 
