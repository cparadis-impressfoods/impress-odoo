...
class LogLineMixin(models.AbstractModel):
    ...
    product_lot_id = fields.Many2one('stock.lot', 'Lot', related='production_id.lot_producing_id', 
    ...
    active_worksheet_field = fields.Char('Active Worksheet Field', compute='_get_active_worksheet_field', store=True)
    ...
    def _get_worksheet_fields(self):
        ...
        worksheet_fields = [k  for k in current_model.fields_get().keys() if 'x_worksheet' in k]
    ...
    def create(self, vals_list):
        if 'quality_wizard_id' in self.env.context:
            wizard = self.env['quality.check.wizard'].browse([self.env.context['quality_wizard_id']])
