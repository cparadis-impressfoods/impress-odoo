...
class AccountMove(models.Model):
    ...
    billback_invoice_id = fields.Many2one('account.move', string="Billback Invoice", readonly=True, copy=False)
    billback_bill_id = fields.Many2one('account.move', string="Billback Bill", readonly=True, copy=False)
    ...
    def action_billback(self):
        if not self.billback_partner_id:
            ...
            raise UserError(_("You can not create billback for a draft or cancelled invoice."))
        elif self.move_type not in ['in_invoice', 'entry']:
            raise UserError(_("Cannot billback move of type %s" % self.move_type))
        else:
            self.billback_invoice_id = self.env['account.move'].with_context(default_move_type='out_invoice').create({
                'partner_id': self.billback_partner_id.id,
                'billback_bill_id': self.id
                })
            for line in self.line_ids:
                if line.price_total != 0:
                    self.env['account.move.line'].with_context(default_move_id=self.billback_invoice_id.id).create({
